...
│  Target:  ApplicationFrameHost | 计算器       │
│  Reason:  target looks destructive           │
│                                              │
│  auto-deny in 27 s                           │
│                                              │
│              [ Allow ]   [ Deny ]            │
...
退出码：`0` 允许 / `1` 拒绝 / `2` 超时自动拒绝 / `3` 弹窗不可用（退回待确认状态）。

**为什么这比 `confirm: true` 强**：`confirm` 是模型自己填的参数，模型可以绕过；
而对话框**只有人手点击才能放行**——模型无法伪造一次人类点击。

**开关**（`approval-toggle.ps1`，双击同目录的 `审批开关.cmd`）：
